...
@router.get("", response_model=SearchResult, name="search:search-query:v2")
...
async def search(
    ...
    print(registry.json())
    search_result = await search_repo.search_v2(query=q, user=current_user)
    return search_result


@router.get(
    "/unranked", response_model=RankingRequest, name="search:search-query:v2:unranked"
)
async def search(
    q: Optional[str] = "",
    search_repo: SearchRepository = Depends(SearchRepository),
